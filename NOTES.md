...
- **tatar** — `SETS` (8 тем, `[татарское, русское, эмодзи]`, ~99 слов: беседа, семья, животные, еда, цвета, числа, тело, природа); режим «Учить» (`#studyScreen`, карточки-перевёртыши) и «Квиз» (`#gameScreen`, направление tr/rt чередуется); квиз проходит ВСЕ слова темы. Татарские буквы ә/ө/ү/җ/ң/һ — обычный Unicode. Эмодзи показывается ТОЛЬКО в «Учить»; в квизе скрыт (картинка подсказывала ответ).
