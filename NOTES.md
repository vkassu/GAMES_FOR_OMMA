...
- **krestiki-noliki** («Крестики-нолики») — доска 3×3 (`board[9]`, `0/1/2`), `WIN_LINES` (8 троек индексов), `checkWinner` возвращает `{player,line}` или `null` (ничья — если все клетки заняты и никто не выиграл). Игрок всегда `1=❌` и ходит первым. 3 режима (`MODES.friend/cat/fox`): `friend` — 2 игрока на одном экране; `cat` — `catMove()` случайные ходы; `fox` — `foxMove()` win→block→center→corner→edge, но с 20% шансом идёт в `catMove` (даёт ребёнку шанс победить). Задержка «раздумья» бота 500–950мс (не мгновенный). Экраны: `#startScreen`→`#gameScreen`→ маленький оверлей результата раунда `#roundOverlay` («Ещё раз»/«Хватит») → финал `#endScreen`. Счёт сессии в `session {wins, losses, draws, games, p1, p2}` — сохраняется до нажатия «Хватит» или смены режима, звёзды начисляются в конце сессии. Правила звёзд (бот): 3+ побед → 3⭐, 2 → 2⭐, 1 → 1⭐, 0 но играл → 1⭐; 2 игрока: 3+ игр → 2⭐, 1+ → 1⭐. Ключ звёзд `krestiki`. Фиолетовая палитра (`--accent:#6366f1`), победная линия подсвечивается жёлтым (`.win`) с пульсацией. Подтверждение выхода: «Выйти? Результат не сохранится».
