...
- **labirint** — генерация DFS-с-возвратом (битовая маска N/E/S/W), кратчайший путь BFS; звёзды-собиралки `coins`(Set) + `visited` (след); звёзды финала по отношению `totalMoves/totalOptimal`; 5 лабиринтов; управление: стрелки/WASD/ЦФЫВ, свайпы, экранный крестик `.dpad`.
- **tatar** — `SETS` (6 тем, `[татарское, русское, эмодзи]`, 50 слов); режим «Учить» (`#studyScreen`, карточки-перевёртыши) и «Квиз» (`#gameScreen`, направление tr/rt чередуется); квиз проходит ВСЕ слова темы. Татарские буквы ә/ө/ү/җ/ң/һ — обычный Unicode. Эмодзи показывается ТОЛЬКО в «Учить»; в квизе скрыт (картинка подсказывала ответ).
- **english** — клон tatar: `SETS` те же 6 тем и те же слова (`[английское, русское, эмодзи]`, 50 слов), направление er/re. Синяя палитра (`--accent:#2563eb`), ключ звёзд `english`, иконка 🔤. Эмодзи так же только в «Учить».
